...
function getAutoLabelForGroup(groups = [], groupId, type = 'verse') {
  const occurrence = getGroupOccurrence(
    groups.map((group) => (
      group.id === groupId
        ? { ...group, type: normalizeSongEditorGroupType(type) }
        : group
    )),
    groupId
  )
...
export default function SongEditorModal({ song, onClose, onSave }) {
  ...
  const availableSections = useMemo(
    () => groups.map((group) => ({ id: group.id, label: resolveSongEditorGroupLabel(group), color: getGroupColor(groups, group.id, group.type) })),
    [groups]
  )
      ...
      const nextGroups = current.map((group) => {
        ...
        if (isCustomSongEditorGroupType(normalizedType)) {
          return {
            ...group,
            type: normalizedType,
            label: shouldAutoLabel ? CUSTOM_GROUP_DEFAULT_LABEL : sanitizeCustomGroupLabel(group.label),
          }
        }
        ...
        return {
          ...group,
          type: normalizedType,
          label: shouldAutoLabel ? getAutoLabelForGroup(
            current.map((entry) => (
                        ...
                        <div className="px-3 py-3 flex flex-col gap-2">
                          {customGroup ? (
                            ...
                            <div>
                              ...
                              />
                              <p className="mt-1 text-[11px] leading-4" style={{ color: 'var(--text-tertiary)' }}>
                                Section type controls color and numbering. You can still rename this section manually.
                              </p>
                            </div>
                          )}
