...
function FontFamilyButton({ value, onChange, width }) {
  return (
    <PopoverMenuButton
      title="Font family"
      label={value || FONT_OPTIONS[0]}
      width={width}
      popoverWidth={Math.max(172, width + 42)}
    >
      {({ close }) => (
...
function FontSizePresetButton({ value, onChange, width = 72, integrated = false }) {
  ...
  return (
    <div data-editor-toolbar="true" className="shrink-0">
      ...
      >
        {!integrated ? (
          <span className="truncate text-[12.5px] font-medium">{value} pt</span>
        ) : null}
        <ChevronDown size={13} />
      </button>
      {open ? (
        <PopoverShell
          popoverRef={popoverRef}
          triggerRef={triggerRef}
          width={integrated ? 84 : width + 28}
        >
          <div className="flex flex-col gap-1 max-h-64 overflow-auto">
            {FONT_SIZE_DISPLAY_PRESETS.map((preset) => (
              <MenuOption
                key={preset}
                active={Number(preset) === Number(value)}
                onClick={() => {
                  onChange(preset);
                  setOpen(false);
                }}
...
export default function Toolbar({ onPresent, onTogglePanel, presenterPanelOpen }) {
  ...
  const activeTextBoxId =
    getCurrentOrSavedTextBoxId() ||
    selectedTextBoxIds[selectedTextBoxIds.length - 1] ||
    primaryTextBox?.id ||
    null;
  ...
  const bodyFontSizeInternal = getRenderedBodyFontSize(
    activeTextBox?.body,
  ...
  const editorFontSizeInternal = activeEditor
    ? hasSelectedEditorText &&
      Number.isFinite(editorSnapshot?.fontSize) &&
      editorSnapshot.fontSize > 0
  ...
  const verticalAlignLabel =
    activeVerticalAlign === 'top' ? 'Top' : activeVerticalAlign === 'bottom' ? 'Bottom' : 'Middle';
  ...
  function applyTextStyle(styleProps, inlineCommand = null, value = null) {
    ...
    if (
      ...
      editorStyleProps.textDecoration =
        [nextStyle.underline ? 'underline' : null, nextStyle.strikethrough ? 'line-through' : null]
          .filter(Boolean)
          .join(' ') || 'none';
  ...
  function applyTextBoxFrameStyle(frameProps) {
    ...
    if (!selectedSectionId || !selectedSlideId || !targetIds?.length) return;
    updateSlideTextBoxes(selectedSectionId, selectedSlideId, (boxes) =>
      boxes.map((box) => (targetIds.includes(box.id) ? { ...box, ...frameProps } : box))
    );
    setEditorTick((tick) => tick + 1);
  }
  ...
  function applyFontSizeValue(value) {
    ...
    const editor = restoreInlineSelection({ focus: false });
    if (
      hasInlineTextSelection(editor) &&
      applyEditorInlineStyle({ fontSize: `${internalValue}px` }, editor)
    ) {
      persistInlineEditor(editor);
      setEditorTick((tick) => tick + 1);
      return;
    }
    applyWholeTextOverride({ size: internalValue }, { fontSize: `${internalValue}px` }, [
      'fontSize',
    ]);
  }
  ...
  function nudgeFontSize(direction) {
    const baseSize =
      editorFontSize || internalToDisplayFontSize(style.size || DEFAULT_TEXT_STYLE.size);
    const next = Math.max(
      MIN_FONT_SIZE_DISPLAY,
      Math.min(MAX_FONT_SIZE_DISPLAY, Math.round(baseSize + direction))
    );
  ...
  function handleVerticalAlign(next) {
    if (selectedSectionId && selectedSlideId)
      updateSlideStyle(selectedSectionId, selectedSlideId, { valign: next }, activeTextBoxIds);
    setEditorTick((tick) => tick + 1);
  }
  ...
  function handleHighlightColorChange(value) {
    ...
    if (
      hasInlineTextSelection(editor) &&
      applyEditorInlineStyle(next === 'transparent' ? {} : { backgroundColor: next }, editor, {
        stripProps: ['backgroundColor'],
        skipWrapperWhenEmpty: next === 'transparent',
      })
    ) {
      persistInlineEditor(editor);
      setEditorTick((tick) => tick + 1);
    ...
    if (editor && canInlineFormat(editor)) {
      editor.innerHTML = slideBodyToHtml(slideBodyToPlainText(editor.innerHTML));
      applyEditorWholeTextStyle(next === 'transparent' ? {} : { backgroundColor: next }, editor, {
        stripProps: ['backgroundColor'],
        skipWrapperWhenEmpty: next === 'transparent',
      });
      persistInlineEditor(editor);
    ...
    applyWholeTextOverride({ highlightColor: next }, {}, ['backgroundColor']);
  }
  ...
  function handleClearFormatting() {
    ...
    if (selectedSectionId && selectedSlideId)
      updateSlideStyle(selectedSectionId, selectedSlideId, DEFAULT_TEXT_STYLE, activeTextBoxIds);
  }
          ...
          <div className="flex min-w-0 flex-1 gap-1 overflow-hidden items-center">
            <Group title="Text">
              <FontFamilyButton
                ...
                onChange={(value) => {
                  ...
                  if (
                    hasInlineTextSelection(editor) &&
                    applyEditorInlineStyle({ fontFamily: value }, editor)
                  ) {
                    persistInlineEditor(editor);
                    setEditorTick((tick) => tick + 1);
                    return;
                  }
                  applyWholeTextOverride({ fontFamily: value }, { fontFamily: value }, [
                    'fontFamily',
                  ]);
                }}
              />
              ...
              <InlineStyleButton
                ...
                active={editorState.bold || style.bold}
                onClick={() => applyTextStyle({ bold: !style.bold }, 'bold')}
              />
              <InlineStyleButton
                icon={Italic}
                title="Italic (Cmd/Ctrl+I)"
                active={editorState.italic || style.italic}
                onClick={() => applyTextStyle({ italic: !style.italic }, 'italic')}
              />
              <InlineStyleButton
                icon={Underline}
                title="Underline (Cmd/Ctrl+U)"
                active={editorState.underline || style.underline}
                onClick={() => applyTextStyle({ underline: !style.underline }, 'underline')}
              />
              {!hideEditSecondaryLabels && (
                <InlineStyleButton
                  icon={Strikethrough}
                  title="Strikethrough"
                  active={editorState.strike || style.strikethrough}
                  onClick={() =>
                    applyTextStyle({ strikethrough: !style.strikethrough }, 'strikeThrough')
                  }
                />
              )}
              {!hidePrimaryLabels && (
                <CommandButton
                  ...
                  compact
                  collapseLabel={hideEditSecondaryLabels}
                />
              )}
            </Group>
            ...
            <Group title="Paragraph" grow noDivider>
              <InlineStyleButton
                ...
                onClick={() => applyTextStyle({ align: 'left' }, 'justifyLeft')}
              />
              <InlineStyleButton
                icon={AlignCenter}
                title="Align Center"
                active={activeAlign === 'center'}
                onClick={() => applyTextStyle({ align: 'center' }, 'justifyCenter')}
              />
              <InlineStyleButton
                icon={AlignRight}
                title="Align Right"
                active={activeAlign === 'right'}
                onClick={() => applyTextStyle({ align: 'right' }, 'justifyRight')}
              />
              {!hideEditSecondaryLabels && (
                <InlineStyleButton
                  icon={AlignJustify}
                  ...
                  onClick={() => applyTextStyle({ align: 'justify' }, 'justifyFull')}
                />
              )}
              ...
              >
                {({ close }) => (
                  <div className="flex flex-col gap-1">
                    <MenuOption
                      active={activeVerticalAlign === 'top'}
                      onClick={() => {
              ...
              {collapseEditColorsToPalette ? (
                <CombinedColorButton
                  ...
                  highlightColor={
                    editorHighlightColor === 'transparent' ? 'transparent' : editorHighlightColor
                  }
                ...
                <>
                  <div className="flex items-center gap-1.5 shrink-0 min-w-0">
                    {hideEditColorLabels ? (
                      ...
                      <InlineTinyLabel>Text</InlineTinyLabel>
                    )}
                    <ColorPickerButton
                      title="Text color"
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 min-w-0">
                    {hideEditColorLabels ? (
                      <InlineTinyIconLabel icon={Highlighter} title="Highlight color" />
                    ) : (
                      <InlineTinyLabel>Highlight</InlineTinyLabel>
                    )}
                    <ColorPickerButton
                      title="Highlight color"
                      value={
                        editorHighlightColor === 'transparent'
                          ? 'transparent'
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 min-w-0">
                    {hideEditColorLabels ? (
                      <InlineTinyIconLabel icon={Square} title="Textbox fill color" />
                    ) : (
                      <InlineTinyLabel>Fill</InlineTinyLabel>
                    )}
                    <ColorPickerButton
                      title="Textbox fill color"
          ...
          <>
            <Group title="Slides">
              <CommandButton
                ...
                primary
                collapseLabel={hidePrimaryLabels}
              />
              <CommandButton
                icon={Type}
                label="Text Box"
                title="Add Text Box"
                onClick={() => addSlideTextBox(selectedSectionId, selectedSlideId)}
                disabled={!canAddTextBox}
                collapseLabel={hidePrimaryLabels}
              />
              <CommandButton
                icon={Copy}
            ...
            <Group title="Insert">
              ...
              >
                {({ close }) => (
                  <div className="flex flex-col gap-1">
                    <MenuOption
                      onClick={() => {
                        importMediaToSelectedSlide('image');
                        close();
                      }}
                    >
                    ...
                    <MenuOption
                      onClick={() => {
                        importMediaToSelectedSlide('video');
                        close();
                      }}
                    >
              ...
              <CommandButton
                icon={FileText}
                label="Announcement"
                title="Add Announcement Section"
