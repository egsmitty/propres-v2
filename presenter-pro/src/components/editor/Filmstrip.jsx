...
export default function Filmstrip({ width = 224 }) {
  ...
  useEffect(() => {
    function handleMouseMove(e) {
      ...
      if (dragCandidate && !activeSlideDrag) {
        ...
        if (Math.hypot(dx, dy) >= 6) {
          ...
          const nextPosition = computeGhostPosition(
            e.clientX,
            e.clientY,
            width,
            dragCandidate.anchor
          );
          setDragGhost({
            slide: dragCandidate.slide,
            anchor: dragCandidate.anchor,
            ...nextPosition,
          });
      ...
      setDragGhost((prev) =>
        prev ? { ...prev, ...computeGhostPosition(e.clientX, e.clientY, width, prev.anchor) } : prev
      );
      const nextTarget = findSlideDropTarget(
        e.clientX,
        e.clientY,
    ...
    function handleMouseUp() {
      ...
      if (dragging && target) {
        const isSameSpot =
          dragging.sectionId === target.sectionId && dragging.slideId === target.targetSlideId;
  ...
  async function insertSongAtIndex(songId, index) {
    ...
    setSlideSelection(
      newSection.id,
      newSection.slides[0]?.id ?? null,
      newSection.slides[0]?.id ? [newSection.slides[0].id] : []
    );
  }
    ...
    mutate((sections) =>
      sections.map((sec) => ({
        ...
        slides: sec.slides.map((sl) =>
          ids.has(sl.id) && !isMediaSlide(sl) ? applySlideAppearanceSnapshot(sl, appearance) : sl
        ),
      }))
    );
  }
    ...
    if (metaToggle) {
      ...
      if (anchorIdx >= 0 && thisIdx >= 0) {
        ...
        setSlideSelection(
          sectionId,
          slide.id,
          allSlides.slice(start, end + 1).map((sl) => sl.id)
        );
      } else {
        setSlideSelection(sectionId, slide.id, [slide.id]);
        anchorSlideRef.current = slide.id;
  ...
  function handleFilmstripKeyDown(event) {
    ...
    event.preventDefault();
    const primary = allSlides.find((entry) => entry.id === selectedSlideId) || allSlides[0];
    setSlideSelection(
      primary.sectionId,
      primary.id,
  ...
  function renderSongDropZone(index) {
    ...
    return (
      <div
        ...
        onDrop={(event) => {
          void handleSongDropZoneDrop(event, index);
        }}
        style={{
          height: active ? 18 : 10,
          padding: '0 10px',
          ...
          return (
            <React.Fragment key={section.id}>
              ...
              <div
                ...
                style={{
                  opacity:
                    dragOverSection === section.id && dragSection.current !== section.id ? 0.6 : 1,
                  outline:
                    dragOverSection === section.id && dragSection.current !== section.id
              ...
              >
                <SectionHeader
                  ...
                  onEditSong={
                    originalSection.type === 'song'
                      ? () => handleEditSong(originalSection)
                      : undefined
                  }
                />
                            ...
                            >
                              <FilmstripSlide
                                ...
                                mediaDropHighlighted={
                                  ...
                                  (mediaDropTargetSlideId &&
                                    effectiveSelectedSlideIds.length > 1 &&
                                    effectiveSelectedSlideIds.includes(slide.id))
                                }
                                onMediaDragOver={(event) =>
                                  handleMediaSlideDragOver(event, originalSection.id, slide.id)
                                }
                                onMediaDrop={(event) => {
                                  void handleMediaSlideDrop(event, originalSection.id, slide.id);
                                }}
