...
import {
  ...
  getSongSectionGroupsAndArrangement,
...
export default function Canvas() {
  ...
  const songSectionData = useMemo(
    () =>
      section?.type === 'song' && resolvedSongId
        ? getSongSectionGroupsAndArrangement(section)
        : null,
  ...
  const primaryTextBoxId =
    selectedTextBoxIds[selectedTextBoxIds.length - 1] || selectedTextBoxIds[0] || null;
  ...
  const { width: nativeW, height: nativeH } = getPresentationDimensions(
    presentation || slide || undefined
  );
    ...
    function handlePointerMove(event) {
      ...
      if (state.type === 'marquee') {
        const rect = normalizeRect(
          state.originX,
          state.originY,
          state.originX + pointerX,
          state.originY + pointerY
        );
      ...
      if (state.type === 'resize') {
        ...
        if (modifierSymmetric) {
          const updated = resizeBoxFromCenter(
            box,
            state.handle,
            pointerX,
            pointerY,
        ...
        if (handleCode.includes('e'))
          width = clamp(box.width + pointerX, MIN_TEXT_BOX_WIDTH, nativeW - x);
        if (handleCode.includes('s'))
          height = clamp(box.height + pointerY, MIN_TEXT_BOX_HEIGHT, nativeH - y);
      ...
      if (state.type === 'marquee') {
        ...
        if (rect && (rect.width > 3 || rect.height > 3)) {
          const touched = textBoxes
            .filter((box) =>
              rectsIntersect(rect, { x: box.x, y: box.y, width: box.width, height: box.height })
            )
            .map((box) => box.id);
  ...
  useEffect(() => {
    function handleKeyDown(event) {
      ...
      if (isEditing && meta) {
        ...
        if (lower === 'b') {
          event.preventDefault();
          if (!runEditorCommand('bold'))
            useEditorStore
              .getState()
              .updateSlideStyle(
                selectedSectionId,
                selectedSlideId,
                { bold: !primaryTextBox?.textStyle?.bold },
                selectedTextBoxIds
              );
          return;
        }
        if (lower === 'i') {
          event.preventDefault();
          if (!runEditorCommand('italic'))
            useEditorStore
              .getState()
              .updateSlideStyle(
                selectedSectionId,
                selectedSlideId,
                { italic: !primaryTextBox?.textStyle?.italic },
                selectedTextBoxIds
              );
          return;
        }
        if (lower === 'u') {
          event.preventDefault();
          if (!runEditorCommand('underline'))
            useEditorStore
              .getState()
              .updateSlideStyle(
                selectedSectionId,
                selectedSlideId,
                { underline: !primaryTextBox?.textStyle?.underline },
                selectedTextBoxIds
              );
          return;
        }
      ...
      if (meta && event.key.toLowerCase() === 'c') {
        event.preventDefault();
        setTextBoxClipboard(
          JSON.parse(JSON.stringify(textBoxes.filter((box) => selectedTextBoxIds.includes(box.id))))
        );
        return;
      }
      ...
      if (meta && ['l', 'e', 'r', 'j'].includes(event.key.toLowerCase())) {
        ...
        useEditorStore
          ...
          .updateSlideStyle(
            selectedSectionId,
            selectedSlideId,
            { align: map[event.key.toLowerCase()] },
            selectedTextBoxIds
          );
        return;
      }
      if (event.shiftKey && event.key === 'F3' && primaryTextBox) {
        ...
        updateSlideBody(
          selectedSectionId,
          selectedSlideId,
          cycleCase(primaryTextBox.body || '', nextIndex),
          primaryTextBox.id
        );
      }
    }
    ...
    caseModeIndex,
    duplicateSlideTextBoxes,
    isEditing,
    mediaOnlySlide,
    primaryTextBox,
    removeSlideTextBoxes,
    selectedSectionId,
    selectedSlideId,
    selectedTextBoxIds,
    setEditingSlide,
  ...
  function handleTextBoxMouseDown(event, textBox) {
    ...
    if (event.shiftKey) {
      ...
      setSelectedTextBoxIds((current) =>
        current.includes(textBox.id)
          ? current.filter((id) => id !== textBox.id)
          : [...current, textBox.id]
      );
      return;
    }
    ...
    const nextIds =
      alreadySelected && selectedTextBoxIds.length ? selectedTextBoxIds : [textBox.id];
            ...
            <ResizeHandles
              onBegin={(event, handle) =>
                beginInteraction(event, 'resize', {
                  box,
                  handle,
                  corner: ['resize_nw', 'resize_ne', 'resize_se', 'resize_sw'].includes(handle),
                })
              }
            />
  ...
  return (
    <div data-tour="canvas" className="flex-1 min-h-0 flex flex-col overflow-hidden bg-bg-app">
        ...
        <SongOrderTray
          ...
          onRemoveEntry={(index) => {
            ...
            applySongArrangement(
              songSectionData.arrangement.filter((_, entryIndex) => entryIndex !== index)
            );
          }}
      ...
      <div
        ...
        onContextMenu={(event) => {
          event.preventDefault();
          setMenu({ x: event.clientX, y: event.clientY, target: 'slide' });
        }}
      >
        <div ref={viewportRef} className="min-h-full w-full flex items-center justify-center">
          {slide ? (
            <div
              ...
              onDragOver={(event) => {
                if (!isMediaLibraryDrag(event) || !slide || !selectedSectionId || !selectedSlideId)
                  return;
                event.preventDefault();
              ...
              onDrop={async (event) => {
                ...
                const mediaId = Number(
                  event.dataTransfer.getData('application/presenterpro-media-id')
                );
                if (!Number.isFinite(mediaId)) return;
                await applyDroppedMediaToTargets(mediaId, [
                  { sectionId: selectedSectionId, slideId: selectedSlideId },
                ]);
              }}
            >
              <div
                ...
                {mediaOnlySlide ? (
                  ...
                  <>
                    {snapGuides.vertical !== null && (
                      <GuideLine direction="vertical" position={snapGuides.vertical} />
                    )}
                    {snapGuides.horizontal !== null && (
                      <GuideLine direction="horizontal" position={snapGuides.horizontal} />
                    )}
                    ...
                    {metric && primaryTextBox && (
                      <div
                        style={{
                          ...INDICATOR_STYLE,
                          left: primaryTextBox.x + 8,
                          top: primaryTextBox.y - 72,
                        }}
                      >
            ...
            >
              <div>
                ...
                <p className="text-sm leading-6">
                  Add sections from the available list above to build this presentation&apos;s
                  version of the song.
                </p>
              </div>
            </div>
          ...
          items={
            menu.target === 'textbox'
              ? [
                  ...
                  { divider: true },
                  {
                    label: 'Duplicate Text Box',
                    onClick: () =>
                      duplicateSlideTextBoxes(selectedSectionId, slide.id, selectedTextBoxIds),
                  },
                  {
                    label: 'Delete Text Box',
                    onClick: () => {
                      removeSlideTextBoxes(selectedSectionId, slide.id, selectedTextBoxIds);
                      setSelectedTextBoxIds([]);
                    },
                    danger: true,
                  },
              ...
              : [
                  ...
                  {
                    label: `Set ${section ? getSectionTypeLabel(section.type) : 'Section'} Background`,
                  ...
                  {
                    label: 'Paste Slide',
                    onClick: () => pasteSlideAfterSelected(),
                    disabled: !slideClipboard,
                  },
                  { label: 'Clear Slide', onClick: () => clearSelectedSlide() },
                  {
                    label: 'Delete Slide',
                    onClick: () => deleteSelectedSlideFromCurrentPresentation(),
                    danger: true,
                  },
                ]
          }
...
const RESIZE_HANDLES = [
  ...
  {
    handle: 'resize_n',
    style: { top: -6, left: '50%', transform: 'translateX(-50%)', cursor: 'ns-resize' },
  },
  { handle: 'resize_ne', style: { top: -6, right: -6, cursor: 'nesw-resize' } },
  {
    handle: 'resize_e',
    style: { top: '50%', right: -6, transform: 'translateY(-50%)', cursor: 'ew-resize' },
  },
  { handle: 'resize_se', style: { bottom: -6, right: -6, cursor: 'nwse-resize' } },
  {
    handle: 'resize_s',
    style: { bottom: -6, left: '50%', transform: 'translateX(-50%)', cursor: 'ns-resize' },
  },
  { handle: 'resize_sw', style: { bottom: -6, left: -6, cursor: 'nesw-resize' } },
  {
    handle: 'resize_w',
    style: { top: '50%', left: -6, transform: 'translateY(-50%)', cursor: 'ew-resize' },
  },
];
      ...
      style={
        direction === 'vertical'
          ? {
              ...
              left: position,
              width: 1,
              background: 'rgba(74,124,255,0.88)',
              pointerEvents: 'none',
            }
          : {
              position: 'absolute',
