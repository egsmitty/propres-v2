...
function getSavedEditorRange(editor) {
  if (!editor || !savedEditorSelection?.range || !savedEditorSelection?.editor?.isConnected)
    return null;
...
export function getEditorSelectionSnapshot(editor = getCurrentOrSavedSlideTextEditor()) {
  ...
  if (!savedEditorSelection?.editor?.isConnected || savedEditorSelection.editor !== editor)
    return null;
  return (
    savedEditorSelection.snapshot || buildSelectionSnapshot(editor, getSavedEditorRange(editor))
  );
}
...
function unwrapEmptyFormattingNodes(editor) {
  ...
  nodes.reverse().forEach((node) => {
    const hasAttributes = [...node.attributes].some(
...
export function applyEditorInlineStyle(
  styleProps,
  editor = getCurrentOrSavedSlideTextEditor(),
  options = {}
) {
  ...
  const entries = Object.entries(styleProps || {}).filter(
...
export function applyEditorWholeTextStyle(
  styleProps,
  editor = getCurrentOrSavedSlideTextEditor(),
  options = {}
) {
...
export function applyEditorBoxStyle(
  styleProps,
  editor = getCurrentOrSavedSlideTextEditor(),
  options = {}
) {
