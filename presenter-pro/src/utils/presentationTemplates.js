    ...
    return createSection('song', index, {
      ...
      slides: [
        makeBodySlide(
          'song',
          'Song Missing',
          'This template song could not be found in the library.'
        ),
      ],
    });
  }
...
function findSongByBuiltInKey(songLibrary = [], builtInKey) {
  return (
    songLibrary.find(
      (song) => song.builtInKey === builtInKey || song.built_in_key === builtInKey
    ) || null
  );
}
...
export const PRESENTATION_TEMPLATES = [
  {
    ...
    description:
      'Announcements, four built-in hymns, and sermon placeholders with real media backgrounds.',
    async buildPresentation({ ensureMedia, songLibrary = [] } = {}) {
      ...
      return {
        ...
        sections: [
          ...
          buildSongSectionFromLibrarySong(
            findSongByBuiltInKey(songLibrary, BUILT_IN_HYMN_IDS.amazingGrace),
            1,
            songBackgroundOne?.id ?? null
          ),
          buildSongSectionFromLibrarySong(
            findSongByBuiltInKey(songLibrary, BUILT_IN_HYMN_IDS.allCreatures),
            2,
            songBackgroundTwo?.id ?? null
          ),
          buildSongSectionFromLibrarySong(
            findSongByBuiltInKey(songLibrary, BUILT_IN_HYMN_IDS.howGreatThouArt),
            3,
            songBackgroundThree?.id ?? null
          ),
          buildSermonPlaceholderSection(4),
          buildSongSectionFromLibrarySong(
            findSongByBuiltInKey(songLibrary, BUILT_IN_HYMN_IDS.greatIsThyFaithfulness),
            5,
            generalBackground?.id ?? null
          ),
        ],
      };
    },
  ...
  },
  {
    ...
    description:
      'A polished sample service with announcements, worship, sermon notes, and a featured flow.',
    async buildPresentation({ ensureMedia, songLibrary = [] } = {}) {
      const announcementsMedia = ensureMedia ? await ensureMedia(SAMPLE_MEDIA_LIBRARY[0]) : null;
      const generalBackground = ensureMedia ? await ensureMedia(SAMPLE_MEDIA_LIBRARY[1]) : null;
      const songBackgroundOne = ensureMedia ? await ensureMedia(SAMPLE_MEDIA_LIBRARY[2]) : null;
  ...
  },
  {
    ...
    async buildPresentation({ ensureMedia } = {}) {
      ...
      return {
        title: 'Announcement Loop',
        sections: [buildAnnouncementsSection(0, announcementsMedia?.id ?? null)],
      };
    },
  },
  {
        ...
        createSection('announcement', 0, {
          ...
          slides: [
            makeBodySlide(
              'announcement',
              'Welcome',
              "Welcome to Prayer Night\nLet's quiet our hearts before the Lord",
              { size: 50 }
            ),
          ],
        }),
