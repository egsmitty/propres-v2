...
function getTargetSections(presentation, targets = []) {
  return targets
    .map(
      (target) => presentation?.sections?.find((section) => section.id === target.sectionId) || null
    )
    .filter(Boolean);
}
...
export async function applyDroppedMediaToTargets(mediaId, targets = []) {
    ...
    state.mutateSections((currentSections) =>
      currentSections.map((section) => ({
        ...
        slides: section.slides.map((slide) =>
          targetSlideIds.has(slide.id) ? { ...slide, backgroundId: mediaId } : slide
        ),
      }))
    );
    return true;
  ...
  if (result.action === 'group') {
    state.mutateSections((currentSections) =>
      currentSections.map((section) =>
        uniqueSectionIds.includes(section.id) ? { ...section, backgroundId: mediaId } : section
      )
    );
    return true;
  }
