...
| E1b rgba literals → alpha tokens · hover handlers → `hover:` classes      | **recommended, not started** — 149 `rgba(…)` literals in JSX (E1 banned hex only); `onMouseEnter` handlers mutate `element.style`, needs a hover capture first                                                                                                                  | —       |
...
| C2 renderer Content-Security-Policy                                       | **recommended, not started** — Electron warns "Insecure Content-Security-Policy" in every renderer; a CSP meta/header for the built app, behind the E2E suite                                                                                                                   | —       |
