...
| F0 song-part vocabulary naming | done — `SONG_PART_TYPES` / `getSongPart` / `getSongPartColor`, so song parts stop colliding with `SECTION_TYPE_META` / `normalizeSectionType`; the `normalizeSectionType('verse') → 'announcement'` coercion is pinned by a test, not fixed | #96 |
| A5 manual verification → E2E | done — three specs that type on the keyboard (`e2e/autosaveTyping.spec.ts`) instead of sending `app:command`, plus the delete-on-discard branch; the charter's rule is that a finding ends in a mechanical check or it regrows | #98 |
| Home template row parity | done — `overflow-x-auto` also computes `overflow-y: auto`, clipping the card's 34–38px shadow; the card now stretches like the New tab's grid cell. Four Home baselines recaptured on the runner, the other 48 byte-identical | #108 |
| Save keeps the selection | done — saving no longer routes through `setPresentation` (which resets selection and clears undo); `syncSavedPresentation` added. Recorded as an A5 finding, hit within minutes of real use | #109 |
| A6 version history | **in flight** — `tasks/plan-A6-version-history.md`; time-thinned retention, restore-is-undoable, Version History modal, disabled while presenting. THE INVARIANT: the newest version always equals the committed row | — |
