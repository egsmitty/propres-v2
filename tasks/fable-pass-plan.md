...
| DB2 database lifecycle (audit MAIN-B10/B11/B12/B14/B15)                   | done — first-launch seed extracted to `electron/db/seed.js` and wrapped in one `db.transaction(...)` (MAIN-B10); `will-quit` now checkpoints the WAL and closes the handle, guarded and error-swallowing (MAIN-B11); the migration runner refuses a database from a newer build with `NewerSchemaVersionError`, before any backup or migration (MAIN-B12); `npm run dev` gets its own `presenterpro-dev.db` via a pure `getDbPath`/`resolveDbFileName` (MAIN-B14); media import's per-file full-table scan replaced with an indexed `findMediaByCanonicalPath` (MAIN-B15, the index already existed); 20 new cases, 18 red on `main` | see PR  |
