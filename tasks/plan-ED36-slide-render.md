...
- [x] 5. **Fidelity E2E — lands green; its red is analytical.** `e2e/renderFidelity.spec.ts`
      ...
      Enter, wait for `[data-slide-editing]`), `showPresenterPanel(page)`, then
      click "Expand All" and wait for the first `[data-slide-render="thumbnail"]`
      — sections start collapsed (`Filmstrip.jsx:360`) and a collapsed section
      mounts no `FilmstripSlide`; the first CI run timed out on that locator —
...
| Preconditions for conditional UI              | Todo 5: presentation open with slide 1 selected, the presenter panel shown (the live preview and grid exist only then) and the Service Order expanded via "Expand All" (sections start collapsed and mount no thumbnails — the first CI run failed on this); Todo 10 adds the output window and a live slide; `lines >= 2` asserted |
